...
| Visibility | [`Range.Hidden`](https://learn.microsoft.com/en-us/office/vba/api/excel.range.hidden), [`Worksheet.Visible`](https://learn.microsoft.com/en-us/office/vba/api/excel.worksheet.visible), [`XlSheetVisibility`](https://learn.microsoft.com/en-us/office/vba/api/excel.xlsheetvisibility) | sheet visibility was inspect-only | delivered: row/column hide and unhide, normal/veryHidden sheet hide/show, last-visible/active-sheet protection, exact mixed-state rollback |
| Workbook links and calculation | [`Workbook.LinkSources`](https://learn.microsoft.com/en-us/office/vba/api/excel.workbook.linksources), `UpdateLink`/`ChangeLink`/`BreakLink`, `Application.Calculation`, `CalculateFull(Rebuild)` | absent | delivered: link inventory/refresh/re-point/break with dependent-formula capture, manual/automatic/semiautomatic + full/fullRebuild, readback and restore (VBA/macros stay forbidden) |
| Advanced objects | `SparklineGroups`, `SlicerCaches`, `Styles`, `Range.GoalSeek`, `Window.Split`, `Workbook.Protect` | absent | delivered: sparklines, slicers, cell styles, goal seek, split panes, passwordless workbook protection; winLoss/password workbooks fail closed |
