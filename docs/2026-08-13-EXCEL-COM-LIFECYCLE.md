...
현재 상태(0.4.20): 이 문서는 당시 측정이다. 이후 호스트 크래시 경로에서 DocBridge 소유
`EXCEL.EXE` 잔류가 관측되었다. 저장된 소유 통합문서의 정상 disconnect/파이프 정리는
지원한다. 사용자 Excel은 강제 종료하지 않는다. 아래 잔류 없음 결과를 현재 보장으로
읽지 마십시오. 현재 계약은 [RELEASE-0.4.20.md](RELEASE-0.4.20.md)를 따른다.
