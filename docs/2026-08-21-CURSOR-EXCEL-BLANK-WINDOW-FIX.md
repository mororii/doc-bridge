...
현재 상태(0.4.20): 이 문서의 0.4.17 수용 기준(status/context가 새 Excel을 만들지 않음,
파일 열기 실패 뒤 빈 소유 인스턴스 정리, 사용자 Excel 미종료)은 그대로 둔다. 호스트
크래시 뒤 소유 `EXCEL.EXE` 잔류는 별도로 관측된 한계이며, 잔류 프로세스 해결을 현재
보장으로 읽지 마십시오. 현재 계약은 [RELEASE-0.4.20.md](RELEASE-0.4.20.md)를 따른다.
