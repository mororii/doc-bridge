...
생성 결과는 `releases\DocBridge-0.4.22-win-x64.zip`입니다. ZIP에는 .NET 8 `win-x64` 런타임, 한글 COM 격리 worker, Codex 플러그인/로컬 marketplace, Claude/Kimi/Cursor MCP 설정 병합기, 한글 보안 모듈 등록기, 진단기와 제거기가 모두 들어갑니다.
...
예상 버전은 `0.4.22`이며 검증 끝에 `모두 통과`가 나와야 합니다. 배포 무결성 검사에는 `doc-bridge-hwp-worker.exe`도 포함됩니다.
